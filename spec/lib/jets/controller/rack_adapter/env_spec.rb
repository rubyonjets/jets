describe Jets::Controller::RackAdapter::Env do
  let(:rack_env) { Jets::Controller::RackAdapter::Env.new(event, context) }
  let(:context) { nil }
  ...
  context "books list" do
    ...
    it "convert" do
      ...
      expect(env['QUERY_STRING']).to eq("a=1&b=2&c%5B%5D=4")
      expect(env['PATH_INFO']).to eq("/books/list")
      expect(env['REMOTE_ADDR']).to eq("69.42.1.180, 54.239.203.100")
      expect(env['REQUEST_URI']).to eq("https://uhghn8z6t1.execute-api.us-east-1.amazonaws.com/books/list?a=1&b=2&c%5B%5D=4")
