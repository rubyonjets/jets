...
describe Jets::Cfn::TemplateMappers::ChildMapper do
  let(:map) do
    Jets::Cfn::TemplateMappers::ChildMapper.new("#{Jets.tmp_build}/templates/#{Jets.config.project_namespace}-posts-controller.yml", "s3-bucket")
  end

  describe "map" do
    it "contains info for app stack resource" do
      expect(map.path).to eq "#{Jets.tmp_build}/templates/#{Jets.config.project_namespace}-posts-controller.yml"
