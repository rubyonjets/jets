...
describe Jets::Cfn::TemplateMappers::ApiGatewayDeploymentMapper do
  let(:map) do
    Jets::Cfn::TemplateMappers::ApiGatewayDeploymentMapper.new("#{Jets.tmpdir}/templates/#{Jets.config.project_namespace}-api-gateway-deployment.yml", "s3-bucket")
  end

  describe "map" do
    it "contains info for app stack resource" do
      expect(map.path).to eq "#{Jets.tmpdir}/templates/#{Jets.config.project_namespace}-api-gateway-deployment.yml"
