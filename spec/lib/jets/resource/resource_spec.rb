describe Jets::Resource do
  ...
  context "simple resource with no replacements" do
    let(:replacements) { {} }
    let(:definition) do
      {
        "RestApi": {
  ...
  context "resource with replacements" do
    let(:replacements) do
    ...
    it "cloudformation format" do
      ...
      # properties under EventPattern has specialized dasherized and pascalized casing
