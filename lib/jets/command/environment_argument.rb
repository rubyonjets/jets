...
module Jets
  module Command
    module EnvironmentArgument # :nodoc:
      ...
      private
        def extract_environment_option_from_argument(default_environment: Jets::Command.environment)
          if options[:environment]
            self.options = options.merge(environment: acceptable_environment(options[:environment]))
          else
            self.options = options.merge(environment: default_environment)
          end
          # JETS_ENV needs to be set before config/application is required.
