...
# Global overrides for Jetsbda processing
$stdout.sync = true
# This might seem weird but we want puts to write to stderr which is set in
# the node shim to write to stderr.  This directs the output to Jetsbda logs.
# Printing to stdout can managle up the payload returned from Jetsbda function.
...
class Jets::Process::BaseProcessor
  ...
  def initialize(event, context, handler)
    # assume valid json from Jetsbda
