  ...
  class Application
    module Bootstrap
      ...
      initializer :load_active_support, group: :all do
        ENV["JETS_DISABLE_DEPRECATED_TO_S_CONVERSION"] = "true" if config.active_support.disable_to_s_conversion
      ...
      initializer :initialize_logger, group: :all do
        Jets.logger ||= config.logger || begin
          ...
          logger.warn(
            "Jets Error: Unable to access log file. Please ensure that #{path} exists and is writable " \
            "(i.e. make it writable for user and group: chmod 0664 #{path}). " \
            "The log level has been raised to WARN and the output directed to STDERR until the problem is fixed."
          )
          logger
        end
        Jets.logger.level = ActiveSupport::Logger.const_get(config.log_level.to_s.upcase)

        unless config.consider_all_requests_local
          Jets.error.logger = Jets.logger
        end
      end

      # Initialize cache early in the stack so turbines can make use of it.
      initializer :initialize_cache, group: :all do
        ...
        unless Jets.cache
          Jets.cache = ActiveSupport::Cache.lookup_store(*config.cache_store)

          if Jets.cache.respond_to?(:middleware)
            config.middleware.insert_before(::Rack::Runtime, Jets.cache.middleware)
          end
        end
      end
      ...
      initializer :setup_once_autoloader, after: :set_eager_load_paths, before: :bootstrap_hook do
        autoloader = Jets.autoloaders.once
