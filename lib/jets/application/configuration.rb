...
require "jets/engine/configuration"

require "action_dispatch/middleware/host_authorization"

module Jets
  class Application
    class Configuration < ::Jets::Engine::Configuration
      include Defaults
      ...
      attr_accessor :allow_concurrency, :asset_host, :autoflush_log,
                    ...
                    :log_tags, :turbines_order, :relative_url_root, :secret_key_base,
      ...
      def initialize(*)
        ...
        if Jets.env.development?
          @hosts = ActionDispatch::HostAuthorization::ALLOWED_HOSTS_IN_DEVELOPMENT +
            ENV["JETS_DEVELOPMENT_HOSTS"].to_s.split(",").map(&:strip)
        else
          @hosts = []
        end
        @host_authorization                      = {}
        ...
        @turbines_order                          = [:all]
        @relative_url_root                       = ENV["JETS_RELATIVE_URL_ROOT"]
      ...
      # {configuration guide}[https://guides.rubyonjets.org/configuring.html]
      # for the default values associated with a particular version.
      def load_rails_defaults(target_version)
      ...
      def paths
        @paths ||= begin
          ...
          paths.add "log",                with: "log/#{Jets.env}.log"
          paths.add "public"
      ...
      def load_database_yaml # :nodoc:
        if path = paths["config/database"].existent.first
          require "jets/application/dummy_erb_compiler"
      ...
      # +:disabled+. +:disabled+ tells Jets not to deal with sessions.
      #
        ...
        if new_session_store
          if new_session_store == :active_record_store
            begin
              ...
              raise "`ActiveRecord::SessionStore` is extracted out of Jets into a gem. " \
      ...
      def annotations
        Jets::SourceAnnotationExtractor::Annotation
      end
      ...
      private
        def default_credentials_content_path
          if credentials_available_for_current_env?
            root.join("config", "credentials", "#{Jets.env}.yml.enc")
          else
        ...
        def default_credentials_key_path
          if credentials_available_for_current_env?
            root.join("config", "credentials", "#{Jets.env}.key")
          else
        ...
        def credentials_available_for_current_env?
          File.exist?(root.join("config", "credentials", "#{Jets.env}.yml.enc"))
        end
    end
  end
end
