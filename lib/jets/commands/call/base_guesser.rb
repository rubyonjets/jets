...
module Jets::Commands::Call
  class BaseGuesser
    ...
    def function_name
      ...
      @provided_function_name = @provided_function_name.sub("#{Jets.project_namespace}-", "")

      code_path = class_name.underscore.gsub('/','-')
      function_name = [Jets.project_namespace, code_path, method_name].join('-')
      generated_function_name(function_name)
    end

    def generated_function_name(function_name)
      if function_name.size > Jets::Cfn::Resource::Lambda::Function::MAX_FUNCTION_NAME_SIZE # name generated by CloudFormation
