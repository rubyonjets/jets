...
module Jets
  module Command
    class CredentialsCommand < Jets::Command::Base # :nodoc:
      ...
      private
        def credentials
          Jets.application.encrypted(content_path, key_path: key_path)
        end
        ...
        def missing_credentials_message
          if credentials.key.nil?
            "Missing '#{key_path}' to decrypt credentials. See `bin/jets credentials:help`"
          else
            "File '#{content_path}' does not exist. Use `bin/jets credentials:edit` to change that."
          end
        end
