...
module Jets
  class Server < ::Rack::Server
    class Options
      def parse!(args)
        Jets::Command::ServerCommand.new([], args).server_options
      end
    end
    ...
    def set_environment
      ENV["JETS_ENV"] ||= options[:environment]
    end
    ...
    def start(after_stop_callback = nil)
      ...
      setup_dev_caching
      # TODO: should this be commented out? Think it needs to be because Lambda will
      # not run through the server...
      # However, need to test puma and see if this will surface errors when there's an app error.
    ...
    private
      def setup_dev_caching
        if options[:environment] == "development"
          Jets::DevCaching.enable_by_argument(options[:caching])
        end
      end

      def create_tmp_directories
        %w(cache pids sockets).each do |dir_to_make|
          FileUtils.mkdir_p(File.join(Jets.root, "tmp", dir_to_make))
        end
      end
      ...
      def log_to_stdout
        ...
        console.formatter = Jets.logger.formatter
        console.level = Jets.logger.level

        unless ActiveSupport::Logger.logger_outputs_to?(Jets.logger, STDOUT)
          Jets.logger.extend(ActiveSupport::Logger.broadcast(console))
        end
      end
  ...
  module Command
    class ServerCommand < Base # :nodoc:
      ...
      class_option :port, aliases: "-p", type: :numeric,
        desc: "Runs Jets on the specified port - defaults to 8888.", banner: :port
      class_option :binding, aliases: "-b", type: :string,
        desc: "Binds Jets to the specified IP - defaults to 'localhost' in development and '0.0.0.0' in other environments'.",
        banner: :IP
      ...
      private
        ...
        def environment
          options[:environment] || Jets::Command.environment
        end

        def restart_command
          "jets server #{@original_options.join(" ")} --restart"
        end
        ...
        def self.banner(*)
          "jets server -u [thin/puma/webrick] [options]"
        end
        ...
        def rack_server_suggestion(server)
          if server.in?(RACK_SERVERS)
            <<~MSG
              ...
              Run `jets server --help` for more options.
            MSG
          else
        ...
        def print_boot_information(server, url)
          say <<~MSG
            ...
            => Jets #{Jets.version} application starting in #{Jets.env} #{url}
