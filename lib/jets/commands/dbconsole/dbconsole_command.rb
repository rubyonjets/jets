...
module Jets
  class DBConsole
    ...
    def start
      ENV["JETS_ENV"] ||= @options[:environment] || environment
      ...
      when "sqlite3"
        ...
        args << File.expand_path(db_config.database, Jets.respond_to?(:root) ? Jets.root : nil)
    ...
    def environment
      Jets.respond_to?(:env) ? Jets.env : Jets::Command.environment
    end
    ...
    private
      def configurations # :doc:
        # require APP_PATH
        ActiveRecord::Base.configurations = Jets.application.config.database_configuration
