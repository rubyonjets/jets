...
require "jets/command/helpers/editor"

module Jets
  module Command
    class EncryptedCommand < Jets::Command::Base # :nodoc:
      include Helpers::Editor

      class_option :key, aliases: "-k", type: :string,
        default: "config/master.key", desc: "The Jets.root relative path to the encryption key"
      ...
      private
        ...
        def change_encrypted_file_in_system_editor(file_path, key_path)
          Jets.application.encrypted(file_path, key_path: key_path).change do |tmp_path|
        ...
        def missing_encrypted_message(key:, key_path:, file_path:)
          if key.nil?
            "Missing '#{key_path}' to decrypt data. See `bin/jets encrypted:help`"
          else
            "File '#{file_path}' does not exist. Use `bin/jets encrypted:edit #{file_path}` to change that."
          end
        end
    end
  end
