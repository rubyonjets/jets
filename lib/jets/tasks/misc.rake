...
desc "List versions of all Jets frameworks and the environment"
...
namespace :time do
  ...
  namespace :zones do
    ...
    # desc 'Displays names of US time zones recognized by the Jets TimeZone class, grouped by offset. Results can be filtered with optional OFFSET parameter, e.g., OFFSET=-6'
    task :us do
      build_time_zone_list ActiveSupport::TimeZone.us_zones
    end

    # desc 'Displays names of time zones recognized by the Jets TimeZone class with the same offset as the system local time'
    task :local do
