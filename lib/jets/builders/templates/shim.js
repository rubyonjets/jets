...
const downloadAndExtract = async (s3_key, download_path, folder_dest) => {
...
const downloadTmp = async () => {
  var start = Date.now();
  var key, download_path, folder_dest;

  key = `jets/code/${BUNDLED_ZIP}`;       // jets/code/bundled-checksum.zip
  download_path = `/tmp/${BUNDLED_ZIP}`;  // /tmp/bundled-checksum.zip
  folder_dest = '/tmp/bundled';
  var bundled = downloadAndExtract(key, download_path, folder_dest);

  key = `jets/code/${RACK_ZIP}`;       // jets/code/rack-checksum.zip
  download_path = `/tmp/${RACK_ZIP}`;  // /tmp/rack-checksum.zip
  folder_dest = '/tmp/rack';
  var rack = downloadAndExtract(key, download_path, folder_dest);
