...
const coldStart = "yes"; // this only runs on cold start, must be on outside scope
...
function request(event, handler, callback) {
  ...
  client.on('close', function() {
    ...
    // the ruby_server.rb should always return an Object guaranteed, so we should not
    // have to do this, but doing it just in case in the future it changes.
    var resp;
    try {
      resp = JSON.parse(stdout_buffer)
    } catch(e) {
      // if json cannot be parse assume simple text output intended
      // log("WARN: error parsing json, assuming plain text is desired.")
      resp = stdout_buffer;
    }

    // At this point, resp is guaranteed to be an object or a string.
    // If resp is a string, then resp["headers"] == undefined
    // Add custom jets-warm header to the response if it's a web response.
    if (resp["headers"]) {
      if (typeof coldStart !== 'undefined') {
        resp["headers"]["jets-warm"] = "no"; // cold start
      } else {
        resp["headers"]["jets-warm"] = "yes"; // coldStart is undefined which means the function is warmed
      }
    }
    // log(resp);
