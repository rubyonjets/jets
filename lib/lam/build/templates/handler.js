...
module.exports.create = (event, context, callback) => {
  ...
  // export PROCESSOR_COMMAND=ruby_wrapper_mac.rb

  const ruby_bin = process.env.RUBY_BIN || "bin/ruby"
  const processor_command = process.env.PROCESSOR_COMMAND || "lam process controller"

  var processor_args = ruby_wrapper.split(' ')
  var event_args = [JSON.stringify(event), JSON.stringify(context), "posts.create"]
  var args = processor_args.concat(event_args)
  // lam process controller [event] [context] [handler]
  var ruby = spawn(ruby_bin, args);

  // string concatation in javascript is faster than array concatation
  // http://bit.ly/2gBMDs6
  var stdout_buffer = ""; // stdout buffer
  // In the ruby_wrapper.rb we do NOT call puts directly and write to stdout
